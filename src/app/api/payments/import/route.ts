...
import { rowToPaymentData } from "@/lib/csv";
import { parseImportFile } from "@/lib/parse-import";
import { purchaseOrderNoIdMap } from "@/lib/links";
import type { PaymentCreateManyInput } from "@/generated/prisma/models/Payment";

type CreateData = Omit<PaymentCreateManyInput, "id" | "createdAt" | "updatedAt">;
...
export async function POST(request: NextRequest) {
  ...
  if (!user.permissions.importPayments) {
  ...
  const poMap = await purchaseOrderNoIdMap();

  const valid: CreateData[] = [];
  const skipped: { row: number; reason: string }[] = [];

  rows.forEach((row, index) => {
    const data = rowToPaymentData(row);
    ...
    valid.push({
      ...
      totalPaymentDone: data.totalPaymentDone,
      balanceAmount: data.balanceAmount,
      purchaseOrderId: data.purchaseOrderNo
        ? poMap.get(data.purchaseOrderNo)
        : undefined,
    });
  });
  ...
  let created = 0;
  if (valid.length > 0) {
    const result = await prisma.payment.createMany({ data: valid });
